...
Seems pretty simple. As a user, these two are simply "the add function" to me. When I call `add(5, 10)`, the compiler looks at the arguments (two integers) and matches the call to the first definition. When I call `add(5.5f, 2.1f)`, it matches it to the second.
...
From the POV of a linker, when you have two different pieces of code are both labeled `add`, I have no way to tell them apart and which one to jump to when a call to `add()` is made. So when I see two symbols with the same name, I'm gonna yell at the programmer and spit it as "multiple definition error".
...
As a compiler, we need way to convince the linker that "hey, these two `add()` functions are totally different" even though they look the same.

Since the linker only wants unique symbols, our problem simplifies a bit {em} we need a way to transform identifiers into unique symbols. We need a way to be able to transform our beautiful `int add(int x, int y)` into a unique symbol. We wish to be able to _mangle_ our `add()` function. How do we do that?
...
Look at that, our integer-based and float-based `add()` functions were mangled to `_Z3addii` and `_Z3addff` respectively, just like what we came up with. 

What about Clang? We'll compile it with x86-64 clang 6.0.1 using `-O3` flag:
...
Looks like our name mangling strategy is consistent.

Okay cool, what about MSVC?
...
Interesting... looks like our `add()` functions were mangled into `?add@@YAHHH@Z` and `?add@@YAMMM@Z` instead of what we saw earlier.
...
Next we have the function name itself `add`, but notice it doesn't prefix the length of the symbol like `3` in GCC. Instead, it reads until it hits a terminator. The `@@` acts as the terminator for the name (and in other cases provides namespace/class information).
