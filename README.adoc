...
:cpp: C++
:em: {sp}--{sp}
:gh-vs123: https://github.com/vs-123
...
* **Regex** {em} Beyond just using it, I've implemented a PCRE-compatible engine called {gh-vs123}/regen[regen] from scratch in C99, in order to understand how it works deep under the hood. This has given me deep insights on not just regex, but also non-finite automata and backtracking
...
* **Python** {em} I use Python as a scripting language, as a glue language. I use it mostly for writing quick scripts that are slightly more complex for bash, quick prototypes of algorithms, visualising data and making statistical observations and verification, like that of {gh-vs123}/ystar[ystar]
...
* {gh-vs123}/regen[regen] {em} A PCRE-compatible, regular-expressions engine written in C99. Supports various features including capture groups, backreferences, zero-width lookaround assertions, character classes, and more. No external libraries were used
* {gh-vs123}/ystar[ystar] {em} A xorshift64* PRNG implementation in C99 *with my own custom constants*, tested and verified with various statistical tests including *Wald-Wolfowitz Runs Test* and *Lag-1 Correlation Test*. *PLOTS AVAILABLE IN README*
* {gh-vs123}/bpvm[bpvm] {em} A simple, fully-functional https://esolangs.org/wiki/BytePusher[BytePusher] VM implementation in C99. *SCREENSHOTS AVAILABLE IN README*
* {gh-vs123}/halloc[halloc] {em} A dead-simple, thread-safe, general-purpose, explicit-freelist heap allocator library written in C99
* {gh-vs123}/barc[barc] {em} A lexicographical permutation-based BWT + local frequency-adaptive MTF transform + multi-byte chunked RLE file-compressor archive utility tool written in C99. This project uses no OS-specific/POSIX/external libraries in order to keep it portable
* {gh-vs123}/l-tail[l-tail] {em} A memory-efficient, POSIX-compliant, high-performance, log-scanning utility tool for stream monitoring written in C99.
* {gh-vs123}/dman[dman] {em} An efficient, POSIX-compliant, policy-based directory-pruning utility tool written in C99
* {gh-vs123}/qenv[qenv] {em} A simple, POSIX-compliant, zero-dependency environment variable inspection utility tool designed for quick and memory-efficient filtering across diverse shell environments.
* {gh-vs123}/mbf[mbf] {em} A custom https://en.wikipedia.org/wiki/Brainfuck[BF] implementation, with the addition of *macros*
* {gh-vs123}/dstr[dstr] {em} A dead-simple dynamic-string library written in C89. Supports printf-style formatters for string concatenation, written from scratch
* {gh-vs123}/minesweeper-c[mswpr] {em} Fully functional Minesweeper implementation in C99, using my own {gh-vs123}/ystar[ystar] PRNG for mine generation and placement. *SCREENSHOTS AVAILABLE IN README*
* {gh-vs123}/kernel[kernel] {em} A dead-simple kernel written in C99 with LLVM toolchain
* {gh-vs123}/tegen[tegen] {em} A recursive, stochastic text generator for Rust
* {gh-vs123}/auc[auc] {em} A cross-platform auto-clicker in C99
* {gh-vs123}/royal-hemlock-theme[Royal Hemlock Theme] {em} Soothing royal-blue light-theme for Emacs. *SCREENSHOTS AVAILABLE IN README*
...
!{gh-vs123}/vs-123/blob/7d4c2b0872f05f84c4f8910fa5f1ffdd4c00f860/c_std_int.webp[C standard int]
...
|===
| Aspect | Choice  
| Environment | {gh-vs123}/dotfiles[My Dotfiles] 
| Text Editor | {gh-vs123}/dotfiles/tree/main/dot_emacs[Emacs] 
| OS | https://voidlinux.org/[Void Linux] 
| WM | {gh-vs123}/dotfiles/tree/main/dot_config/openbox[Openbox] 
| Shell | {gh-vs123}/dotfiles/blob/main/dot_zshrc[Zsh]
|===
