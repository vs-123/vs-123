...
* {my-gh}/kalium-void[Kalium Void] {em} My personal, custom Linux distro based on Void Linux. This is my daily driver OS. Utilises link:{my-gh}/kalium-packages[Kalium Packages] repository alongside that of Void, has a modified base, unique identity, built-in Nix package manager, pre-installed essential packages, pre-installed fonts, uses X.Org/X11 display server with openbox WM, uses zsh as default shell instead of dash, uses GRUB for bootloader. Custom installation script is provided.
...
| OS | link:{my-gh}/kalium-void[Kalium Void] 
